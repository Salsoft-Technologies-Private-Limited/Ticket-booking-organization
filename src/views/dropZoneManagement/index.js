...
import { COMPANY, JUMP } from "../../config/constants/api";
import { extractDate } from "../../config/helpers";
import { useDebouncedCallback } from "use-debounce";
import { dropZoneManagementData } from "../../components/Data/data";
...
const StaysManagement = () => {
  ...
  const getDropzoneCompanies = (pageNumber, pageSize, status, from, to, keyword) => {
    setLoading(true);
    Get(COMPANY.getCompanies, token, {
      status,
      from,
      to,
      keyword,
  ...
  const handlePageChange = (pageNumber) => {
    ...
    getDropzoneCompanies(pageNumber, paginationConfig.limit);
  };
  ...
  const columns = [
    ...
    {
      title: "Drop Zone Company Name",
      dataIndex: "dropzoneCompany",
      key: "dropzoneCompany",
    ...
    },
    {
      ...
      render: (value, item, index) => (
        <AiOutlineEye
          ...
          onClick={() =>
            navigate(`/dropZoneManagement/${value}`, {
              state: { dropzoneCompany: item  },
            })
          }
        />
      ),
