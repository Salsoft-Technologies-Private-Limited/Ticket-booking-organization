...
const StaysManagement = () => {
  ...
  const getReservations = (pageNumber, pageSize, status, from, to, keyword) => {
    setLoading(true);
    Get(`${BOOKING.getAllEventBookings}${eventId}`, token, {
      status,
      from,
      to,
      keyword,
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "event",
      key: "event.date",
      render: (value, item, index) => <span>{extractDate(value.date)}</span>,
    },
    {
      title: "Status",
      dataIndex: "status",
    ...
    },
    {
      ...
      dataIndex: "event",
      key: "event.price",
      render: (value, item, index) => "$" + value.price,
    },
    {
      title: "Action",
      dataIndex: "_id",
      ...
      render: (value, item, index) => (
        <AiOutlineEye
          ...
          onClick={() => navigate(`/eventManagement/reservationLogs/${value}`)}
        />
      ),
    },
  ];
